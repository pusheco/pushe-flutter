...
internal class PusheChandler(private val context: Context,
    ...
    private fun triggerEvent(call: MethodCall, result: MethodChannel.Result) {
        try {
            Pushe.getPusheService(co.pushe.plus.inappmessaging.PusheInAppMessaging::class.java)
            ...
            result.error("023", """
                ...
                implementation("co.pushe.plus:inappmessaging:${'$'}latest") in dependencies{}
            """.trimIndent(), null)
            return
        }
        val piamModule = Pushe.getPusheService(co.pushe.plus.inappmessaging.PusheInAppMessaging::class.java)
        if (piamModule == null) {
    ...
    private fun disableInAppMessaging(result: MethodChannel.Result) {
    ...
    private fun enableInAppMessaging(result: MethodChannel.Result) {
    ...
    private fun isInAppMessagingEnabled(result: MethodChannel.Result) {
    ...
    private fun dismissShownInApp(result: MethodChannel.Result) {
        try {
            Pushe.getPusheService(co.pushe.plus.inappmessaging.PusheInAppMessaging::class.java)
        } catch (e: NoClassDefFoundError) {
            result.error("026", """
    ...
    @VisibleForTesting
    fun testInAppMessage(call: MethodCall, result: MethodChannel.Result) {
        ...
        piamModule.testInAppMessage(call.argument("message") ?: "", call.argument("instant")
          ?: false)
    }

    private fun initializeInAppMessagingListeners() {
        try {
            Pushe.getPusheService(co.pushe.plus.inappmessaging.PusheInAppMessaging::class.java)
        } catch (e: NoClassDefFoundError) {
            lg("""
